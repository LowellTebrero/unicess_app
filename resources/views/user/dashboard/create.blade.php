<x-app-layout>
    ...
    <section class="mt-5 m-8 rounded-xl  2xl:h-[87vh] h-[85vh] bg-white text-gray-700">
        ...
        <div class="flex justify-between items-center p-4 xl:py-3 2xl:py-4">
            <h1 class="2xl:text-2xl xl:text-lg text-[.9rem] font-semibold text-slate-600">Upload Proposal <span class="text-red-500 text-xs tracking-wide font-light"> * required fields</span></h1>
        ...
        <form class="pt-2 px-5 2xl:px-10 xl:mt-2 2xl:mt-5 h-[75vh] 2xl:h-[78vh]" id="FormSubmit" action="{{ route('User-dashboard.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="w-full mx-auto 2xl:py-4 py-2 p-4 border relative rounded-md">
            <div class="flex space-x-2 flex-row  w-full">
                <div class="w-1/2">
                    <label class="text-xs block text-slate-600  font-medium mb-2 2xl:text-sm" for="program_id">Program Name <span class="text-red-500">*</span></label>
                    <select id="program_id" class="rounded-md text-xs w-full border-zinc-300  py-2 px-3" name="program_id" value="{{ old('program_id') }}" required>
                ...
                <div class="w-full">
                    <label class="text-xs block text-slate-600  font-medium mb-2 2xl:text-sm" for="project_title">Proposal Title <span class="text-red-500">*</span></label>
                    <input class="border-zinc-300 text-xs  appearance-none border rounded w-full  py-2 px-3 text-slate-600 leading-tight focus:outline-none" name="project_title" id="project_title" type="text" value="{{ old('project_title') }}" placeholder="project title" required>
            ...
            <div class="flex xl:justify-between mt-4 space-x-2 items-center w-full">
                <div class="w-full">
                    <label class="text-xs block text-slate-600  font-medium mb-2 2xl:text-sm">PROJECT PROPOSAL (PDF) <span class="text-red-500">*</span></label>
                    <input class="custom-file bg-white border-zinc-300 text-[.7rem] appearance-none border  rounded w-full px-3 text-slate-600 leading-tight focus:outline-none" name="proposal_pdf" id="proposal_pdf" type="file" required>
                    @error('proposal_pdf') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                </div>

                <div class="w-full">
                    <label class="text-xs block text-slate-600  font-medium mb-2 2xl:text-sm">SPECIAL ORDER (PDF) <span class="text-red-500">*</span></label>
                    <input class="bg-white border-zinc-300 text-[.7rem] appearance-none border  rounded w-full px-3 text-slate-600 leading-tight focus:outline-none" name="special_order_pdf" id="special_order_pdf" type="file" required>
                    @error('special_order_pdf') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                </div>

                <div class="w-1/2">
                    <label class="text-xs block text-slate-600  font-medium 2xl:text-sm">Started Date<span class="text-red-500">*</span></label>
                    <input required class="border-zinc-300 text-xs  appearance-none border  rounded w-full py-2 mt-2 px-3 text-slate-600  leading-tight focus:outline-none" value="{{ old('started_date') }}" name="started_date" id="started_date" type="date">
                    @error('started_date') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                </div>

                <div class="w-1/2">
                    <label class="text-xs block text-slate-600  font-medium 2xl:text-sm">Ended Date<span class="text-red-500">*</span></label>
                    <input required class="border-zinc-300 text-xs appearance-none border  rounded w-full py-2 mt-2 px-3 text-slate-600  leading-tight focus:outline-none" value="{{ old('finished_date') }}" name="finished_date" id="finished_date" type="date">
            ...
            <div class="flex space-y-4 flex-col w-full pt-3">
                    ...
                    <div class="flex space-x-4 w-full" style="display: none" id="leaderDiv">
                        ...
                        <table class="w-full">
                            ...
                            <tr class="text-sm text-gray-500 sticky top-0 bg-white">
                                <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left">Project leader</th>
                                <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left">Role of Leader</th>
                                <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left w-[11rem] 2xl:w-[10rem]">Location</th>
                            </tr>
                            </thead>
                            ...
                            <tr>
                                <td class="pr-4 xl:w-[21rem] 2xl:w-[30rem]">
                                    ...
                                    <select name="leader_id" class="rounded-md text-xs w-full  border-zinc-300" value="{{ old('leader_id') }}" id="leader">
                                ...
                                <td class="pr-4  xl:w-[21rem] 2xl:w-[30rem]">
                                    <select onchange="yesnoCheck(this)" id="leader_member_type" name="leader_member_type" value="{{ old('leader_member_type') }}" class="rounded-md text-xs w-full border-zinc-300">
                ...
                <div class="w-full overflow-x-auto h-[26vh] xl:h-[23vh] 2xl:h-[35vh] bg-slate-100" style="display: none" id="memberDiv">

                    <table id="table" class="w-full">
                        <thead>
                        <tr class="text-sm text-gray-500 sticky top-0 bg-white">
                            <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left">Member Name</th>
                            <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left">Member Type</th>
                            <th class="text-xs  text-slate-600 font-medium mb-2 2xl:text-sm text-left w-[11rem] 2xl:w-[10rem]">Action</th>
                        </tr>
                        </thead>
                        ...
                        <tr>
                            <td class="pr-4 xl:w-[21rem] 2xl:w-[30rem]">
                                <select name="member[0][id]" class="rounded-md text-xs w-full border-zinc-300" id="member">
                                    @foreach($members as $id => $name )<option value="{{ $id }}" @if ($id == old('member_id[0][id]')) selected="selected" @endif >{{ $name }}</option> @endforeach
                                </select>
                            </td>

                            <td class="pr-4  xl:w-[21rem] 2xl:w-[30rem]">
                                <select name="member[0][type]" class="rounded-md text-xs w-full border-zinc-300">
                                    <option value="">Select Participation</option>
                                    @foreach($parts_names as $id => $participation_name )<option value="{{ $participation_name }}" @if ($id == old('member[0][type]')) selected="selected" @endif >{{ $participation_name }}</option> @endforeach
                                </select>
                            </td>

                            <td>
                                <button name="add" id="add" type="button" class="w-full bg-slate-500 rounded-md text-white px-2 py-2  text-xs  border-zinc-300">Add more user</button>
                            </td>
                        </tr>
                    </tbody>
                    </table>
            ...
            <div class="bottom-2 left-0 xl:bottom-7">
                <button class="bg-blue-500 rounded-lg text-white 2xl:text-base xl:text-sm text-xs font-medium py-2 px-4 mt-4 focus:outline-none" type="submit">
                    Submit Proposal
                </button>
            </div>
           </div>
