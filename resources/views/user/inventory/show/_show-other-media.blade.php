    ...
    <div class="mt-10 py-10">
        ...
        <div class="flex flex-wrap">
            @foreach ($proposals->getMedia('otherFile') as $image)
                ...
                <div class="bg-white  flex w-full  xl:w-48 shadow-md rounded-lg hover:bg-slate-200  transition-all m-2 relative ">
                    ...
                    <div x-cloak  x-data="{ 'showModal': false }" @keydown.escape="showModal = false" class="absolute right-0 top-1" >
                        ...
                        <div class="fixed inset-0 z-50  flex items-center justify-center overflow-auto bg-black bg-opacity-50" x-show="showModal">
                            ...
                            <div class="w-1/4 py-4 text-left bg-white rounded-lg shadow-lg" x-show="showModal"
                                ...
                                <!-- content -->
                                <div class="">
                                    <form action="{{route('inventory-rename-media', $image->id)}}" method="POST">
                                        @csrf @method('PUT')
                        ...
                        <div x-cloak x-data="{dropdownMenu: false}" class=" absolute right-0">
                            ...
                            <div x-show="dropdownMenu" class="z-50 absolute right-[-5] py-2 mt-2 bg-white rounded-md shadow-xl w-32" x-on:keydown.escape.window="dropdownMenu = false"  @click.away="dropdownMenu = false" @click="dropdownMenu = ! dropdownMenu"
                                ...
                                <a href={{ route('inventory-download-media', $image->id) }} class="block text-xs px-2 py-2 hover:text-black" x-data="{dropdownMenu: false}">Download</a>

                                @if ( $proposal_member->leader_member_type == !null)
                                <form action="{{ route('inventory-delete-media', $image->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return confirm ('Are you sure?')" >
                                    @csrf @method('DELETE')
                                    <button class="block text-slate-800 text-xs px-2  hover:text-black" type="submit">Delete</button>
                                </form>
                                @endif
                            </div>
                        </div>
                    ...
                    <x-alpine-modal>
                        ...
                        <x-slot name="scripts">
                            <div class="flex p-5 xl:p-3 2xl:p-5  w-full">
                                ...
                                <div>
                                    ...
                                    <img src="{{ asset('img/image-icon.png') }}" class="2xl:w-[2.5rem]" width="40" alt="">
                                    @else
                                    <img src="{{ asset('img/pdf.png') }}" class="2xl:w-[2.5rem]" width="40" alt="">
                                    @endif
                                </div>

                                <div class="text-xs ml-2 text-left">
                                    <span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->file_name, 15) }}</span>
                        ...
                        <x-slot name="title">
                            <span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->file_name) }}</span>
                        </x-slot>

                        <div class="w-[50rem]">
                            @if ($image->mime_type == 'image/jpeg' || $image->mime_type == 'image/png' || $image->mime_type == 'image/jpg')
                                <div class=""><img class="shadow w-full " src="{{  $image->getUrl() }}"></div>
                            @else
                                <div><iframe class="shadow mt-2 w-full h-[80vh]" src="{{  $image->getUrl() }}"></iframe></div>
                            @endif
                        </div>
                    </x-alpine-modal>
                </div>
