    ...
    <section class="2xl:min-h-[87vh] h-[85vh] mt-5 m-8 rounded-lg bg-white text-gray-700">
        ...
        <main>
            <div class="p-7 m-10 rounded border flex xl:flex-row flex-col space-y-5 xl:space-y-0">
                ...
                <div class="w-full relative  flex space-y-5 flex-col sm:flex-row sm:space-y-0 sm:space-x-4  xl:flex-col xl:space-y-5 xl:space-x-0  ">

                    <div class="absolute top-6 right-[10rem]  text-white px-2 py-1 rounded-lg text-sm">

                        <div class="flex space-y-2 flex-col text-gray-700">

                            @if ($ProposalRequest->leader_member_type == !NULL)
                                @if ($leaderMember->isEmpty())
                                <button data-modal-target="default-modal" data-modal-toggle="default-modal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                    Grant Request
                                </button>
                                @else

                                <button disabled data-modal-target="default-modal" data-modal-toggle="default-modal" class="cursor-not-allowed block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                    Request Granted
                                </button>
                                @endif
                        ...
                        <div id="default-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-2xl max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <!-- Modal body -->
                                    <form action={{ route('admin.dashboard.member-request-store') }} method="POST">
                                        @csrf
                                    <div class="p-4 md:p-5 ">

                                        <input type="text" value="{{ $ProposalRequest->proposal_id }}" name="proposal_id" hidden>

                                        @if ($ProposalRequest->leader_member_type == !NULL)
                                        <div class="flex space-x-2">
                                            <div class="w-full">
                                                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Add as Leader</p>
                                                <label>{{ $ProposalRequest->user->name }}</label>
                                                <input type="text" name="leader_id" value="{{ $ProposalRequest->user_id}}" hidden>
                                                @error('project_leader') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                                            </div>

                                            <div class="w-full">
                                                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Role Type</p>
                                                <label>{{ $ProposalRequest->ceso->role_name }}</label>
                                                <input type="text" name="leader_member_type" value="{{ $ProposalRequest->leader_member_type}}" hidden>
                                                @error('role_name') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                                            </div>

                                            <div class="w-full">
                                                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Location</p>
                                                <label>{{ $ProposalRequest->leader_location == NULL ? 'N/A' : $ProposalRequest->location->location_name }}</label>
                                                <input type="text" name="location_id" value="{{ $ProposalRequest->leader_location }}" hidden>
                                        ...
                                        @if ($ProposalRequest->member_type == !NULL)
                                        <div class="flex space-x-2">
                                            <div class="w-full">
                                                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Add as Member</p>
                                                <label>{{ $ProposalRequest->user->name }}</label>
                                                <input type="text" name="member_id" value="{{ $ProposalRequest->user_id}}" hidden>
                                                @error('member_id') <span class="text-red-500  text-xs">{{ $message }}</span> @enderror
                                            </div>

                                            <div class="w-full">
                                                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">Member Type</p>
                                                <label>{{ $ProposalRequest->member_type }}</label>
                                                <input type="text" name="member_type" value="{{ $ProposalRequest->member_type}}" hidden>
                                            </div>
                                        </div>
                                        @endif
                    ...
                    <div class="w-full text-gray-600">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <h1 class="text-xs w-auto">{{ $ProposalRequest->proposal->project_title }}</h1>
                            </div>
