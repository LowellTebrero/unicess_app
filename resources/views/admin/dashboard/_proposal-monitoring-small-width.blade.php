    ...
    <div class="h-full flex space-x-2 mt-7 justify-between ">
        {{--  Proposal Summary  --}}
        <div class="flex flex-col p-3 rounded-lg  bg-white w-full">
            <h1 class="text-gray-700 text-xs 2xl:text-[1.1rem] tracking-wider font-medium">Project Monitoring</h1>
            <div class="justify-between space-x-2 flex mt-2">

                <button
                    class="border border-gray-400 h-[5vh] bg-gradient-to-r from-green-200 to-green-500  w-full rounded-lg p-5  flex space-x-5 items-center relative overflow-hidden  duration-100"
                    type="button" x-data="{}"
                    x-on:click="window.livewire.emitTo('finished-proposal', 'show')">

                        <svg class="fill-white w-[1.5rem]" xmlns="http://www.w3.org/2000/svg" height="50"
                            width="50">
                            <path d="m20 32.4-7.85-7.85 2.4-2.4L20 27.6l13.45-13.45 2.4 2.4Z" />
                        </svg>

                        <h1 class="text-sm xl:text-xl 2xl:text-3xl font-bold text-white">
                        {{ $finishedProposal }}</h1>
                </button>

                <button
                    class="border border-gray-400 h-[5vh] bg-gradient-to-r from-sky-400 to-blue-500 w-full  rounded-lg p-5 flex space-x-5 items-center relative overflow-hidden  duration-100"
                    type="button" x-data="{}"
                    x-on:click="window.livewire.emitTo('ongoing-proposal', 'show')">


                    <svg class="fill-white w-[1.5rem]" xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                        viewBox="0 0 24 24">
                        <path
                    ...
                    <h1 class="text-sm xl:text-xl 2xl:text-3xl font-bold  rounded-3xl text-white">{{ $ongoingProposal }}</h1>
                </button>

                <button
                    class="border border-gray-400 h-[5vh] bg-gradient-to-l from-orange-600 via-orange-400 to-yellow-300   w-full rounded-lg p-5 space-x-5   items-center relative overflow-hidden flex "
                    type="button" x-data="{}"
                    x-on:click="window.livewire.emitTo('pending-proposal', 'show')">



                    <svg class="fill-white w-[1.5rem]" xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 24 24">
                        <path
                    ...
                    <h1 class="text-sm xl:text-xl 2xl:text-3xl text-white font-bold ">{{ $projectProposal }}</h1>
