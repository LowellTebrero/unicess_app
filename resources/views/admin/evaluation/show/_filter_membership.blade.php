<div class="mt-6 ">
    ...
    <div class="mb-4 flex space-x-4">
        <div class="w-full">
            ...
            <input onkeypress="return isNumber(event)" id="membership_university" class=" input-number border-zinc-400 appearance-none border rounded w-full py-2 px-3  leading-tight text-sm focus:outline-none" name="membership_university" value="{{ $evaluation->membership_university }}" type="text">
                                        ...
                                        <x-slot name="scripts">
                                            <div class="flex p-5 xl:p-3 2xl:p-5 ">
                                                ...
                                                <div class="text-xs ml-2 text-left">
                                                    <span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->membership_wide) }}</span>
                                                </div>
                                            </div>
                                        </x-slot>
                                        <x-slot name="title"><span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->membership_wide) }}</span></x-slot>
                                        <div class="w-[70rem]">
                                            <iframe class=" mt-2 w-full h-[80vh]" src="{{ (!empty($image->membership_wide))? url('file/'. $image->path): url('upload/no-image.png') }}"></iframe>
                                        </div>
                                    </x-alpine-modal>
                                </div>
                             @endif
        ...
        <div class="w-full">
            ...
            <input onkeypress="return isNumber(event)" id="membership_college" class=" input-number border-zinc-400 appearance-none border rounded w-full py-2 px-3  leading-tight text-sm focus:outline-none" name="membership_college" value="{{ $evaluation->membership_college }}" type="text">
                                    ...
                                    <x-alpine-modal>
                                        ...
                                        <x-slot name="scripts">
                                            <div class="flex p-5 xl:p-3 2xl:p-5 ">
                                                ...
                                                <div class="text-xs ml-2 text-left">
                                                    <span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->membership_unit) }}</span>
                                                </div>
                                            </div>
                                        </x-slot>
                                        <x-slot name="title"><span class="xl:text-[.7rem] 2xl:text-xs">{{ Str::limit($image->membership_unit) }}</span></x-slot>
                                        <div class="w-[70rem]">
                                            <iframe class=" mt-2 w-full h-[80vh]" src="{{ (!empty($image->membership_unit))? url('file/'. $image->path): url('upload/no-image.png') }}"></iframe>
                                        </div>
                                    </x-alpine-modal>
                                </div>
                                @endif
