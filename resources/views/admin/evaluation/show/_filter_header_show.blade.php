...
<div class="relative">
    ...
    <div type="button" class="transition-all hover:bg-blue-500 fixed z-20 top-[12vh] left-[2rem] lg:top-[13vh] lg:left-[3rem]  xl:top-[13vh] xl:left-[13rem] 2xl:top-[12vh] 2xl:left-[18rem] bg-blue-400 rounded border hover:shadow-2xl shadow-xl hover:border-gray-700 border-black p-2"  onclick="toggleVisibility()" id="userDetailDiv" style="display: none;">
        <div class="transition-all flex flex-col text-sm space-y-1 hover:text-white ">
            ...
            <h1>Username: {{ $evaluation->users->name }}</h1>
            <h1>Email: {{ $evaluation->users->email }}</h1>
            <h1>Role:
            @foreach ($evaluation->users->roles as $role )
                {{ $role->name }}
            @endforeach
            </h1>
            <h1>Faculty: {{ $evaluation->users->faculty->name }}</h1>
        </div>
    </div>
