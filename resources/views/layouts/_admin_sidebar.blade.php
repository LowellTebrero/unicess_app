...
<div class="sidebar-toggle shadow-2xl h-screen bg-gradient-to-t from-blue-500 to-blue-800 transition-all fixed  xl:w-[12rem] 2xl:w-[14rem]">
        ...
        @role('admin')
            ...
            <x-custom-nav-link :href="route('admin.extension-monitoring.index')" :active="request()->routeIs('admin.extension-monitoring.*')">
                <svg class="fill-white mr-2" xmlns="http://www.w3.org/2000/svg" width="35" height="30" viewBox="0 0 24 24">
                    <path fill="#ffffff"
                        d="M4 21q-.425 0-.712-.288T3 20v-1q0-.425.288-.712T4 18q.425 0 .713.288T5 19v1q0 .425-.288.713T4 21m4 0q-.425 0-.712-.288T7 20v-5.5q0-.425.288-.712T8 13.5q.425 0 .713.288T9 14.5V20q0 .425-.288.713T8 21m4 0q-.425 0-.712-.288T11 20v-3.5q0-.425.288-.712T12 15.5q.425 0 .713.288T13 16.5V20q0 .425-.288.713T12 21m4 0q-.425 0-.712-.288T15 20v-5q0-.425.288-.712T16 14q.425 0 .713.288T17 15v5q0 .425-.288.713T16 21m4 0q-.425 0-.712-.288T19 20v-9q0-.425.288-.712T20 10q.425 0 .713.288T21 11v9q0 .425-.288.713T20 21m-6-9.025q-.4 0-.763-.15t-.662-.425L10 8.825l-5.3 5.3q-.3.3-.712.288t-.713-.313q-.275-.3-.262-.712T3.3 12.7l5.275-5.275q.3-.3.663-.437T10 6.85q.4 0 .775.138t.65.437L14 10l5.3-5.3q.3-.3.713-.288t.712.313q.275.3.263.713t-.288.687L15.425 11.4q-.275.275-.65.425t-.775.15" />
                </svg>
            ...
            <x-custom-nav-link :href="route('admin.article-event.index')" :active="request()->routeIs('admin.article-event.*')">
             ...
             <div id="div-6">
            <x-custom-nav-link :href="route('admin.template.index')" :active="request()->routeIs('admin.template.*')">
                <svg class="fill-white mr-2" xmlns="http://www.w3.org/2000/svg" height="35" viewBox="0 96 960 960"
                    width="30">
                    <path
    ...
    @auth
    @php
        $accessRecords = App\Models\CustomizeAdminAccess::all();
    @endphp
    <script>
        var accessRecords = @json($accessRecords);
        document.addEventListener('DOMContentLoaded', function() {
            accessRecords.forEach(function(record) {
                var div = document.getElementById('div-' + record.id);
                if (div) {
                    div.style.display = record.status === 'close' ? 'none' : 'block';
                }
            });
        });
    </script>
