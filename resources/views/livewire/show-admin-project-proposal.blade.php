    ...
    @if ($programID->id == $proposal->program_id)
         <div class="p-10  xl:w-1/4 bg-slate-100 drop-shadow rounded">
             ...
             <h1>Started Date: {{ $proposal->started_date }}</h1>
             <h1>Finished Date {{ $proposal->finished_date }}</h1>
             <h1>Budget Proposed: {{ $proposal->budget_proposed }} </h1>
             <h1>Involve name: {{ $proposal->involve_name }} </h1>
             <h1>Description: {{ $proposal->description }}</h1>
        </div>
        @endif
        @endforeach
    </div>
